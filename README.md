# Yiping — Windows版

## 初回ダウンロード

[Yiping-Launcher.zipをダウンロード](https://github.com/Passione0901/Yiping-Releases/releases/latest/download/Yiping-Launcher.zip)

1. ZIPをダウンロードして、書き込み可能なフォルダへすべて展開します。
2. Launcher.exeを起動します。launcher_config.jsonは同じフォルダに置いてください。
3. 「アップデート」を押してゲーム本体をインストールします。
4. 「ゲーム起動」を押します。

Windows 11 x64向け。.NETの別途インストールは不要です。

## 2回目以降

ゲームを終了し、Launcher.exeを起動してください。公開済みの最新版があれば「アップデート」を押します。最新版ならそのまま「ゲーム起動」を押せます。

アップデートは正式Releaseの最新版が対象です。開発中の未公開変更は取得されません。

## 問題が起きた場合

表示されたエラー、現在のバージョン、操作手順、LauncherLogsフォルダ内の該当ログを開発者へ渡してください。
更新中はGame_backupやupdate-state.jsonを手動で削除しないでください。
Gameフォルダは更新時に置き換わります。個人ファイルを保存しないでください。


## Android

[Android APK download and update instructions](https://github.com/Passione0901/Yiping-Releases/blob/main/ANDROID.md)
